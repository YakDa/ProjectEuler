...
public class NonAbundantSums {
	...
	/*
	 ...
	 is a perfect number. A number n is called deficient if the sum of its proper divisors is less than n and 
	 it is called abundant if this sum exceeds n. As 12 is the smallest abundant number, 1 + 2 + 3 + 4 + 6 = 16, 
	...
	private boolean canBeSumOf2AbundanNum(Integer num)
	{
		for(Integer i = 1; i <= num/2; i++)
		{
			if(isAbundantNum(i) == true && isAbundantNum(num - i) == true)
				return true;
		}
		return false;
	}
	
	private boolean isAbundantNum(Integer num)
	{
		Integer sum = 0;
	...
	public static void main(String[] args) {
		...
		for(Integer i = 1; i <= 28123; i++ )
		{
			if(!nonAbundantSums.canBeSumOf2AbundanNum(i))
				sum = sum + (long)i;
		}
