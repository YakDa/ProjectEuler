...
def destroy():
	...
	p_G.stop()
	p_B.stop()
	# after thread exit, to be safe, set the PIN to be input, in case
	# someone connect this PIN to ground accidentally, it will burn the pin
	# set to input mode, which can stand 3.3V, 5V voltage, so it is considerably
	# safe
	for i in pins:
