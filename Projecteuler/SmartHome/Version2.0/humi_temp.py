...
def writelog(sdata):
	...
	filename = "log/" + (str(datetime.datetime.now()))[:-16] + '.txt'
	...
	def run(self):
		...
		while True:
			...
			if self.action == "display":
				...
				timestamp = (str(datetime.datetime.now()))[:-7]
				humidity, temperature = read_humid_temp()
				sdata = "Time: " + timestamp + ", Humidity: " + str(humidity) + "%,  Temperature: " + str(temperature) + "C`\n"
				writelog(sdata)
			...
			counter = 0
			while ((not self.stopped()) and (counter != self.interval)):
				time.sleep(1)
				counter += 1
