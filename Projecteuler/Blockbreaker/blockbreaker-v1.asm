...
;-----------------------------------------------------------------------------------------
    	...
    	LJMP	SERIAL_ISR
        CSEG    AT  03H               ; Interrupt vector position
        LJMP   PB_INTTERUPT0          ; Jmp to the position of interrupt subroutine
        END 
        END
    	CSEG	AT	0BH
    	LJMP	TIMER_ISR
        ...
        STOPMODE: DB    "Press 'Reset' button to continue!" ; 33 chars, Position X: 24 Y: 12
START:
        MOV A, PCON         ; judge if the bit6 of PCON is 1
        ANL A, #01000000b
        JNZ RECOVER
...
LOOP:
        ...
        LJMP LOOP
;------------------------------------------------------------------------
;   Check stop mode for game
;------------------------------------------------------------------------
CHECK_STOP:
        JNB GAME_STOP, STOP_LABEL1
        CLR GAME_STOP
        LCALL PUSHSTACK
        LCALL INIT_SERIAL
        MOV A, #0CH             ; clear the screen
        LCALL TX_DATA
        MOV A, #0CH
        LCALL TX_DATA
        MOV A, #27              ; reset terminal to initial state
        LCALL TX_DATA
        MOV A, #99
        LCALL TX_DATA
        MOV R7, #33
...
STOP_LABEL0:
        ...
        MOV TAR,#AAh
        MOV TAR,#55h
        ORL PCON, #01000000b    ; set bit6
        MOV TAR,#AAh
        MOV TAR,#55h
        ORL PCON, #00000010b    ; set bit1 to start stop mode
        LCALL POPSTACK
STOP_LABEL1:                
        RET
;-----------------------------------------------------------------------------------------
